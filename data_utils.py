...
def read_data(args, data_type):
  ...
  # shares = ["scene_feat", "video_wh", "scene_grid_strides",
            # "person_boxkey2id", "person_boxid2key"]
  ...
  # Till this point, no batch_scene_feat exist
  return Dataset(data, data_type, vid2name.item(), shared=shared, config=args), vid2name
...
class Dataset(object):
  ...
  def __init__(self, data, data_type, vid2name, config=None, shared=None):
    self.data = data
  ...
  def get_batches(self, batch_size, \
    ...
    for _ in range(num_steps):  # num_step should be batch_idxs length
      ...
      # init batch_scene_feat for the first time
      batch_data.update({
          "batch_obs_scene": new_obs_scene,
          "batch_scene_feat": scene_feat,
      })

      yield batch_idxs, Dataset(batch_data, self.data_type, self.vid2name, shared=self.shared)
...
def encode_other_boxes(person_box, other_box):
    ...
    return [
        ...
        math.log(abs(w_n/float(w_m)) + 1e-2),
        math.log(abs(h_n/float(h_m)) + 1e-2),
    ]


def get_data_feed(batch, data_type='train', N=None):
    """ Giving a batch of data, construct the feed dict """
    ...
    data['obs_scene'] = obs_scene
    data['obs_scene_mask'] = obs_scene_mask
    ...
    for i in range(len(data['obs_boxid'])):
        for j in range(len(data['obs_boxid'][i])):
            ...
            featfile = os.path.join('/home/zhufl/Data2/next-prediction-v2/code/prepare_data/person_appearance_features', data_type, '%s.npy' % boxid)
            obs_person_features[i, j] = np.squeeze(np.load(featfile), axis=0)
    data['obs_person_feat'] = obs_person_features.copy()

    # add other box
    other_boxes_class = np.zeros((N, 8, 15, 10), dtype='float32')
    other_boxes = np.zeros((N, 8, 15, 4), dtype='float32')
    other_boxes_mask = np.zeros((N, 8, 15), dtype='bool')
    for i in range(len(data['obs_other_box'])): # -> num of other boxes
        for j in range(len(data['obs_other_box'][i])): # -> num of seq_len
            ...
            # random shuffle other boxes
            random_other = True
            if random_other:
                random.shuffle(other_box_idxs)
            other_box_idxs = other_box_idxs[:15]

            # get the current person box
            this_person_x1y1x2y2 = data['obs_box'][i][j]

            for k, idx in enumerate(other_box_idxs): # k -> seq_len
                other_boxes_mask[i, j, k] = True

                other_box_x1y1x2y2 = this_other_boxes[idx]

                other_boxes[i, j, k, :] = encode_other_boxes(
                    this_person_x1y1x2y2, other_box_x1y1x2y2)

                # one-hot representation
                box_class = this_other_boxes_class[idx]
                if type(box_class) != int:
                    if sum(box_class) != 0:
                        box_class = np.where(box_class == 1.0)
                        box_class = int(box_class[0][0])
                    else:
                        box_class = 0

                other_boxes_class[i, j, k, box_class] = 1

    data['obs_other_box_class'] = other_boxes_class
    data['obs_other_box'] = other_boxes
    data['obs_other_box_mask'] = other_boxes_mask
    return data, other_boxes
